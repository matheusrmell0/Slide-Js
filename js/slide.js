...
export default class Slide {
  ...
  // Este método ativado apenas no movimentar do mouse, recebe em seu parâmetro o retorno da função updatePosition salvo em uma constante na função onMove o
  // valor atualizado da propriedade (this.dist.movement) do objeto dist. Este valor atualizado é passado em uma propriedade CSS do seletor referenciado na parâmetro da classe
  // O novo valor atualizado do this.dist.movement que foi passado na propriedade translate3d do elemento CSS é atribuido a uma nova propriedade (this.movePosition) do objeto dist
  moveSlide(distX) {
    this.dist.movePosition = distX;
    this.slide.style.transform = `translate3d(${distX}px, 0, 0)`;
  }

  // Método que remove e para a ativação do movimento de mouse e sua função de callback ao soltar o click do mouse
  ...
  // Método que passa o target do eixo X (event.clientX) no movimentar do mouse como parâmetro no método updatePosition e o ativa
